...
import com.lexor.cs.service.ApiSOServiceDetail;
...
@Stateless
@Path("/apiSODetail")
public class SODetailsFacadeREST extends AbstractAPIFacade<ApiSaleOrderDetails> {
    
    private ApiSOServiceDetail service;
        
    private Session mailSession;

    public SODetailsFacadeREST() {
        super(ApiSaleOrderDetails.class);
        this.service = new ApiSOServiceDetail();
    }
    ...
    public int create(ApiSaleOrderDetails entity) throws SQLException {
    ...
    @Path("/{id}")
    @Consumes({MediaType.APPLICATION_JSON})
    public int edit(@PathParam("id") Integer id, ApiSaleOrderDetails entity) throws SQLException {
    ...
    public int remove(@PathParam("id") Integer id, @CookieParam("x-api-token") Cookie cookie) throws SQLException {
        ...
        ApiSaleOrderDetails entity = super.find(id, token);
    ...
    @Path("/{id}")
    @Produces({MediaType.APPLICATION_JSON})
    public ApiSaleOrderDetails find(@PathParam("id") Integer id, @CookieParam("x-api-token") Cookie cookie) throws SQLException {
        String token = TokenHelper.getToken();
        if (cookie != null) {
            token = cookie.getValue();
        }
    ...
    @Path("/{from}/{to}")
    @Produces({MediaType.APPLICATION_JSON})
    public List<ApiSaleOrderDetails> findRange(@PathParam("from") Integer from, @PathParam("to") Integer to) throws SQLException {
